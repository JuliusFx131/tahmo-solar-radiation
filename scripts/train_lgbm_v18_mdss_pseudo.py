"""
TAHMO Solar Radiation — v18 = v16 features + MDSSFTD-derived + pseudo-labels.

Combines:
  • v16's MDSSFTD merge (raw dssf, fdiff, dssf_direct, qflag)
  • NEW MDSSFTD-derived features:
      - ext_mdssftd_kt = dssf / clip(csr_clearsky_ghi, 1, inf)   (MDSSFTD clearness)
      - ext_mdssftd_anomaly = dssf - csr_ghi                     (vs CSR all-sky)
      - ext_mdssftd_clearsky_anomaly = dssf - csr_clearsky_ghi   (vs CSR clear-sky)
      - ext_mdssftd_fdiff_x_csr_ghi = fdiff * csr_ghi            (diffuse irradiance)
      - ext_mdssftd_direct_minus_csr_bhi = dssf_direct - csr_bhi (direct-beam residual)
      - dssf rolling means at 1h / 3h
  • Pseudo-labeling, but now using v10 / v12_pseudo_v10 / v16 as confidence sources
    (v16 is our newest strong model; v8 was older / weaker so we drop it).

Drops v11's m2_aod_* features (proved net-negative LB+0.1 in v11_m2 submission).

Output:
  submissions/lgbm_v18_mdss_pseudo.csv
  submissions/lgbm_v18_mdss_pseudo_oof.csv
  submissions/lgbm_v18_mdss_pseudo_log.txt
"""
...
# Source submissions for the pseudo-label confidence proxy.
# v10, v12_pseudo, v16 — all are strong (LB composite 33.0 / 32.6 / 32.7).
# Old v8/v11 dropped (weaker, would dilute consensus).
SOURCE_SUBS = [
...
# MDSSFTD per-station 15-min files
MDSSFTD_DIR = ROOT / "data" / "satellite" / "mdssftd_per_station"

TARGET = "radiation (W/m2)"
ID     = "ID"
RUN_TAG = "lgbm_v18_mdss_pseudo"
...
def merge_mdssftd(df):
    """Merge LSA-SAF MDSSFTD per-station 15-min files (raw 4 cols)."""
    files = sorted(p for p in MDSSFTD_DIR.iterdir() if p.suffix == ".csv")
    log.info(f"  reading {len(files)} MDSSFTD station files from {MDSSFTD_DIR}")
    parts = []
    for p in files:
    ...
    df = df.merge(mdss, on=["station", "timestamp"], how="left", copy=False)
    log.info(f"  MDSSFTD merged: shape={df.shape}, "
             f"dssf coverage={df['ext_mdssftd_dssf'].notna().mean():.3f}, "
             f"fdiff coverage={df['ext_mdssftd_fdiff'].notna().mean():.3f}")
    return df


def add_mdssftd_derived(df):
    """Derived features that combine MDSSFTD with CSR (the LB-top feature)."""
    eps = np.float32(1.0)
...
def add_mdssftd_rolling(df):
    """1h / 3h rolling means of dssf and its kt, per station (memory-efficient)."""
    df = df.sort_values(["station", "timestamp"]).reset_index(drop=True)
    g_idx = df.groupby("station", sort=False).indices
    for col, short in [("ext_mdssftd_dssf", "mdss_dssf"),
                       ("ext_mdssftd_kt",   "mdss_kt")]:
...
def main():
    ...
    # MDSSFTD merge (raw 4 cols) — do this first so derived features can use
    # both MDSSFTD and CSR columns already present in *_enhanced.csv.
    log.info("Merging MDSSFTD into augmented train + test ...")
    aug_train = merge_mdssftd(aug_train)
    test      = merge_mdssftd(test)
    log.info("Deriving MDSSFTD interaction features (kt, anomalies, fdiff×csr) ...")
    aug_train = add_mdssftd_derived(aug_train)
    test      = add_mdssftd_derived(test)

    aug_train = add_time_features(ffill_cams(aug_train))
    aug_train = add_rolling_features(aug_train)
    aug_train = add_mdssftd_rolling(aug_train)
    test      = add_time_features(ffill_cams(test))
    test      = add_rolling_features(test)
    test      = add_mdssftd_rolling(test)
    ...
    BASE = [
        ...
        # v16: raw MDSSFTD (dssf is #2 feature by gain in v16)
        "ext_mdssftd_dssf", "ext_mdssftd_fdiff",
        "ext_mdssftd_dssf_direct", "ext_mdssftd_qflag",
        # v18: MDSSFTD ⊗ CSR derived features
        ...
        # v18: MDSSFTD rolling means
        "mdss_dssf_mean_1h", "mdss_dssf_mean_3h",
        "mdss_kt_mean_1h",   "mdss_kt_mean_3h",
    ]
    ...
    # Capture original-train IDs in CURRENT (sorted) order — needed for OOF save
    orig_train_ids = aug_train.loc[aug_train["_origin"] == "train", ID].values.copy()
    ...
    # Persist OOF predictions for original-train rows (ID-aligned) so we can use
    # them in future ensembles / splicing.
    oof_orig = oof_pred[orig_all]
    assert len(orig_train_ids) == len(oof_orig), \
        f"OOF length mismatch: {len(orig_train_ids)} vs {len(oof_orig)}"
    oof_df = pd.DataFrame({
        ID: orig_train_ids,
        "TargetMBE":  np.clip(oof_orig, 0, 1361),
        "TargetRMSE": np.clip(oof_orig, 0, 1361),
    })
