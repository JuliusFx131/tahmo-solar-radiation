...
def main():
    ...
    BASE = [
        ...
        "ext_np_precip_corr",
        # v6: temporal neighbours — per (station, hour, doy) rolling mean
        # over ±7/15/30/60 days of training radiation
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        # Free memory between folds — 8 GB cgroup is tight with 130 features.
    ...
    # Save OOF predictions (radiation W/m², per training ID) for stacking
    oof_df = pd.DataFrame({
        ID: train[ID].values,
        "TargetMBE":  np.clip(oof_pred, 0, 1361),
        "TargetRMSE": np.clip(oof_pred, 0, 1361),
    })
    oof_path = SUBS / f"{RUN_TAG}_oof.csv"
    oof_df.to_csv(oof_path, index=False)
    log.info(f"Saved OOF preds for stacking: {oof_path}")
