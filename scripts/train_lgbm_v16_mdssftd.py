"""
TAHMO Solar Radiation — v16 (v10 + LSA-SAF MDSSFTD)
====================================================
Adds to v10:
  • ext_mdssftd_dssf        — LSA-SAF Down-welling Shortwave Flux (15-min, station)
  • ext_mdssftd_dssf_direct — direct-beam component (NaN at night, expected)
  • ext_mdssftd_fdiff       — diffuse fraction (LB leader's signature "Inversion" feature)
  • ext_mdssftd_qflag       — quality flag (numeric — LGBM splits on it)

Why MDSSFTD: it's the lever the LB leader explicitly named. Diffuse fraction
captures aerosol+cloud beam attenuation directly at the station, complementary
to the CAMS/Open-Meteo modelled fluxes already in v10. Native 15-min cadence,
no time-interpolation artefacts.

Everything else from v10 unchanged:
  • CAMS Solar Radiation Timeseries (still the top feature by gain)
  • NASA POWER, Open-Meteo, ERA5, pvlib, CAMS species
  • Forward weather lags, daily aggregates, energy-balance features
  • 6-fold GroupKFold-by-month CV, per-(station, hour) night override
"""
...
def merge_mdssftd(df: pd.DataFrame) -> pd.DataFrame:
    """Merge LSA-SAF MDSSFTD per-station 15-min files onto the combined timeline.
    Native cadence matches train/test (15 min) so this is a direct left-merge."""
    ...
    log.info(f"  MDSSFTD combined: {mdss.shape}")
    before = df.shape
    df = df.merge(mdss, on=["station", "timestamp"], how="left", copy=False)
    coverage = {c: float(df[c].notna().mean())
                for c in ("ext_mdssftd_dssf", "ext_mdssftd_fdiff",
                          "ext_mdssftd_dssf_direct", "ext_mdssftd_qflag")}
    log.info(f"  after merge: {df.shape}  (was {before})  coverage={coverage}")
    return df
...
def main():
    ...
    log.info("Merging LSA-SAF MDSSFTD into full timeline ...")
    full = merge_mdssftd(full)
    ...
    BASE = [
        ...
        # v8: SAME-DAY DAILY AGGREGATES (whole-day max/min/sum/mean per station-date)
        ...
        # v8: ENERGY-BALANCE inversion features (physically motivated)
        ...
        # v10: CAMS Solar Radiation Timeseries (pre-computed 15-min GHI/BHI/DHI/BNI
        # at the station's exact coordinates, all-sky AND clear-sky variants)
        ...
        # v16: LSA-SAF MDSSFTD (the LB leader's signature diffuse-fraction lever).
        # `fdiff` and `dssf_direct` are NaN at night by definition — LGBM handles natively.
        "ext_mdssftd_dssf", "ext_mdssftd_fdiff",
        "ext_mdssftd_dssf_direct", "ext_mdssftd_qflag",
    ]
    ...
    ROLLS = [c for c in train.columns
             ...
             and not c.startswith("ext_fw_")]   # ext_fw_* names collide with these suffixes
    FEATURES = list(dict.fromkeys(BASE + TIME + ROLLS + CATEGORICAL_FEATURES))  # dedupe order-preserved
