"""
TAHMO Solar Radiation — LGBM v9 with pseudo-labeling.

Idea: take v4 / v7 / v8 predictions on the TEST set, find rows where the
three models agree closely (low std across predictions = high confidence),
and add those rows back into training with the consensus prediction as a
fake label. Then re-train v8 on the augmented dataset.

Why this works for interpolation problems:
  • Train and test are interleaved odd/even months at the same stations.
  • When models agree at a test row, the consensus is usually close to truth.
  • Adding those high-confidence rows as training increases the effective
    sample size at the SAME stations and the SAME climate patterns the
    model has to predict for.
  • Original training rows get sample_weight=1.0; pseudo-labels get
    sample_weight=0.5 (down-weighted to limit error amplification).

Confidence rule (configurable):
  • Daytime rows (elev > 0): std(v4_pred, v7_pred, v8_pred) < PSEUDO_STD_THRESH
  • Nighttime rows: skipped (already handled by per-(station, hour) override)

Features: same as v8 (forward-weather + daily aggregates + energy balance).

Run:
  bash /workspace/shell/run_train_v9_pseudolabel.sh

Output:
  submissions/lgbm_v9_pseudolabel.csv
  submissions/lgbm_v9_pseudolabel_log.txt
"""
...
SOURCE_SUBS = [
    SUBS / "lgbm_feat_eng_v4.csv",
    SUBS / "lgbm_v7_fwd.csv",
    SUBS / "lgbm_v8_inversion.csv",
]

# Pseudo-labeling knobs
PSEUDO_STD_THRESH = 15.0        # W/m² — tighter than v9 v1 (30) to control memory + only pick really-confident
PSEUDO_WEIGHT     = 0.5         # weight on pseudo-labelled rows during training
DAYTIME_ELEV_THR  = 5.0         # only pseudo-label rows with elev > this

TARGET = "radiation (W/m2)"
ID     = "ID"
RUN_TAG = "lgbm_v9_pseudolabel"
