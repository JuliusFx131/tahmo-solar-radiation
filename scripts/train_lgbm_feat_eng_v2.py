"""
TAHMO Solar Radiation — LGBM + richer features (v2)
====================================================
Adds to v1:
  • `anomaly_score` per station (from notebook Section K) as a feature —
    lets the model adjust prediction confidence per known-quality station.
  • Rolling means / lags of ext_era5_tcwv (water vapour) and
    ext_era5_blh (boundary layer height). v1 only rolled temp/rh/precip/ssrd/tcc.
  • Per-(station, hour) nighttime override (built from training rows
    with elev≤0), replacing the simpler per-station mean used in v1.
    Captures seasonal daylight-length shifts implicitly: a Mali "hour 19"
    night sample only enters the table from winter rows, matching what
    we'll apply at test time.
...
focused on the things our prior runs didn't have:
  ...
  • Weather lags + rolling stats: temp, humidity, precip, ssrd, tcc, tcwv, blh
    (past 1h / 3h / 24h, computed per-station respecting time order)
  • Recent precip accumulation (past 6h, 24h)
  • Temperature / humidity / precip / ssrd / tcc change vs 1h ago
...
Run:
  bash /workspace/shell/run_train_lgbm_feat_eng_v2.sh

Outputs:
  /workspace/submissions/lgbm_feat_eng_v2.csv
  /workspace/submissions/lgbm_feat_eng_v2_log.txt
"""
...
ID     = "ID"
RUN_TAG = "lgbm_feat_eng_v2"
ANOMALY = PROC / "station_anomaly_scores.csv"
...
def add_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # v2 adds tcwv (total column water vapour) and blh (boundary layer height).
    roll_cols = {
...
def main():
    ...
    # Merge per-station anomaly score (Section K of viz notebook). Stations
    # with high anomaly_score have noisy / partial / drifted sensors; passing
    # this as a feature lets the model temper its predictions for them.
    if ANOMALY.exists():
        anom = pd.read_csv(ANOMALY)[["station", "anomaly_score"]]
        full = full.merge(anom, on="station", how="left")
        log.info(f"  merged anomaly_score for {anom['station'].nunique()} stations")
    else:
        log.warning(f"  {ANOMALY} not found — anomaly_score feature will be NaN")
        full["anomaly_score"] = np.nan
    ...
    FEATURES = BASE + TIME + ROLLS + QUALITY + CATEGORICAL_FEATURES
    log.info(f"  features used: {len(FEATURES)}  "
             f"({len(ROLLS)} rolling/lag, +anomaly_score)")
    ...
    # Built only from training rows where elev<=0, so it implicitly
    # captures seasonal daylight-length shifts: a Mali hour=19 only enters
    # the table from winter rows (because summer rows have elev>0), and
    # we'll only apply it at test where elev<=0 — same season.
