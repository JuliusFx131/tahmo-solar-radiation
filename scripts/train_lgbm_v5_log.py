...
def main():
    ...
    y_train_raw = train[TARGET].values            # raw radiation
    # Train target = log1p(radiation). log1p(0) = 0 so night rows are fine.
    y_train = np.log1p(np.clip(y_train_raw, 0, None))
    X_test  = test[FEATURES]
    groups  = train["timestamp"].dt.month.values

    params = dict(
        objective="regression", metric="rmse",     # RMSE on log space
        learning_rate=0.05, num_leaves=63, min_data_in_leaf=100,
        feature_fraction=0.9, bagging_fraction=0.9, bagging_freq=5,
        verbose=-1, n_jobs=-1, seed=42,
    )

    log.info("Running 6-fold GroupKFold (by month) CV — LOG-SPACE target ...")
    fold_scores = []
    oof_pred_raw = np.zeros(len(train))            # back-transformed
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        pred_log = model.predict(X_train.iloc[va_idx])
        pred_raw = np.expm1(pred_log)              # back to W/m²
        oof_pred_raw[va_idx] = pred_raw
        held_out_month = groups[va_idx][0]
        mbe, rmse, score = score_components(y_train_raw[va_idx], pred_raw)
    ...
    mbe_all, rmse_all, score_all = score_components(y_train_raw, oof_pred_raw)
    log.info(f"OOF (pre bias-correction):  "
             f"|MBE|={mbe_all:.2f}  RMSE={rmse_all:.2f}  score={score_all:.2f}")

    # Bias-correction term: expm1(mean(pred_log)) ≠ mean(expm1(pred_log)).
    # The OOF residual gives us the systematic shift to apply at test time.
    bias_shift = float(np.mean(y_train_raw - oof_pred_raw))
    log.info(f"Estimated bias shift to add to test preds: {bias_shift:+.3f} W/m²")
    ...
    test_pred = np.expm1(test_pred_log) + bias_shift
