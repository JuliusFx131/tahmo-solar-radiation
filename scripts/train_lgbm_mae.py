"""
TAHMO Solar Radiation — LGBM MAE-objective companion (v1)
==========================================================
Identical features to train_lgbm_feat_eng.py but with MAE (regression_l1)
objective. Competition score weighs |MBE| equally with RMSE — MAE-optimised
trees predict the conditional MEDIAN (lower bias variance), usually shaving
|MBE| at the cost of slightly higher RMSE. Used as a companion to ensemble
against the RMSE-optimised model.
...
Run:
  bash /workspace/shell/run_train_lgbm_mae.sh

Outputs:
  /workspace/submissions/lgbm_mae_v1.csv
  /workspace/submissions/lgbm_mae_v1_log.txt
"""
...
def main():
    ...
    # MAE objective + MAE metric. Use l1 ↔ regression_l1.
    params = dict(
